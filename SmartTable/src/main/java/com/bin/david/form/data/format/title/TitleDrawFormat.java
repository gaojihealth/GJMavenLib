...
public class TitleDrawFormat implements ITitleDrawFormat {
    ...
    public int measureWidth(Column column, TableConfig config) {
        ...
        return (int) (paint.measureText(TextUtils.isEmpty(column.getColumnName()) ? "" : column.getColumnName()));
    }
    ...
    private void drawText(Canvas c, Column column, Rect rect, Paint paint) {
        ...
        c.drawText(TextUtils.isEmpty(column.getColumnName()) ? "" : column.getColumnName(), DrawUtils.getTextCenterX(rect.left, rect.right, paint), DrawUtils.getTextCenterY((rect.bottom + rect.top) / 2, paint), paint);
    }
