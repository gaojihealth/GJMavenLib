...
public class TableMeasurer<T> {
    ...
    private int getTableHeight(TableData<T> tableData, TableConfig config) {
        ...
        int titleHeight = 0;
        if (tableData.getChildColumns() != null && tableData.getChildColumns().size() > 0) {
            for (Column column : tableData.getChildColumns()) {
                int _titleHeight = config.isShowColumnTitle() ? (tableData.getTitleDrawFormat().measureHeight(column, config)
    ...
    private int getTableWidth(TableData<T> tableData, TableConfig config) {
        ...
        if (tableData.getChildColumns() != null && tableData.getChildColumns().size() > 0 && tableInfo != null) {
            for (Column column : tableData.getChildColumns()) {
