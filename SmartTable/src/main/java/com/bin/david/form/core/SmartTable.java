...
public class SmartTable<T> extends View implements OnTableChangeListener {
    ...
    private void requestReMeasure() {
        //不是精准模式 且已经测量了
        if (!isExactly && getMeasuredHeight() != 0 && tableData != null && tableData.getTableInfo() != null) {
            if (tableData.getTableInfo().getTableRect() != null) {
                ...
                if (config.getCanScrollVertical()) {
...
//        setMeasuredDimension(measureWidth(widthMeasureSpec), expandSpec);
