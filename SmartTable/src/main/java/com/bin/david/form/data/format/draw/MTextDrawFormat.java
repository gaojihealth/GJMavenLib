...
public class MTextDrawFormat<T> implements IDrawFormat<T> {

    private Map<String, SoftReference<String[]>> valueMap; //避免产生大量对象
    private int color;
    private List<String> mDatas;

    public MTextDrawFormat() {
        valueMap = new HashMap<>();
    }

    public MTextDrawFormat(int color, List<String> datas) {
        valueMap = new HashMap<>();
        this.color = color;
        mDatas = datas;
    }
        ...
        if (mDatas != null && mDatas.size() > 0) {
            if (mDatas.get(cellInfo.row) != null) {
                if (!TextUtils.isEmpty(mDatas.get(cellInfo.row))) {
                    if ((int) Double.parseDouble(mDatas.get(cellInfo.row)) == 1) {
                        paint.setColor(color);
                        setTextPaint(config, cellInfo, paint, color);
    ...
    public void setTextPaint(TableConfig config, CellInfo<T> cellInfo, Paint paint, int color) {
        config.getContentStyle().fillPaint(paint);
        ICellBackgroundFormat<CellInfo> backgroundFormat = config.getContentCellBackgroundFormat();
        if (backgroundFormat != null && backgroundFormat.getTextColor(cellInfo) != TableConfig.INVALID_COLOR) {
            paint.setColor(backgroundFormat.getTextColor(cellInfo));
