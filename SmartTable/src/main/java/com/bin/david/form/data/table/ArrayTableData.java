...
public class ArrayTableData<T> extends TableData<T> {
     ...
     * @return 创建的二维数组表格数据
     */
    public static <T> ArrayTableData<T> create(String tableName, String[] titleNames, String[] titleDatas, T[][] data, IDrawFormat<T> drawFormat) {
        List<Column<T>> columns = new ArrayList<>();
        for (int i = 0; i < data.length; i++) {
            T[] dataArray = data[i];
            Column<T> column = new Column<>(titleNames == null ? "" : (titleDatas!=null?titleDatas[i]:""), null, drawFormat);
            column.setDatas(Arrays.asList(dataArray));
            column.setParentColumnName(titleNames[i]);
            Column parentColumn = new Column(titleDatas == null ? "" : titleNames[i], column);
