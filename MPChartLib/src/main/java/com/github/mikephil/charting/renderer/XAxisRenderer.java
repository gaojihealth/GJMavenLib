...
public class XAxisRenderer extends AxisRenderer {
    ...
    protected void drawLabels(Canvas c, float pos, MPPointF anchor) {
        ...
        for (int i = 0; i < positions.length; i += 2) {
            ...
            if (mViewPortHandler.isInBoundsX(x)) {
                ...
                drawLabel(c, label, x, pos, anchor, labelRotationAngleDegrees,mXAxis.getTextColor());
            }
        }
    }
    ...
    protected void drawLabel(Canvas c, String formattedLabel, float x, float y, MPPointF anchor, float angleDegrees,int color) {
        ...
        mFirstLinePaint.setColor(color);
        mFirstLinePaint.setTextAlign(Align.CENTER);
        mFirstLinePaint.setTextSize(labelHeight);
        mFirstLinePaint.setTypeface(mXAxis.getTypeface());

        Paint mSecondLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mSecondLinePaint.setColor(color);
        mSecondLinePaint.setTextAlign(Align.CENTER);
        mSecondLinePaint.setTextSize(labelHeight);
        mSecondLinePaint.setTypeface(mXAxis.getTypeface());

        Paint mThirdLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
//        mThirdLinePaint.setColor(color);
        mThirdLinePaint.setColor(Color.parseColor("#FA6469"));
        mThirdLinePaint.setTextAlign(Align.CENTER);
//        mSecondLinePaint.setTextSize(Utils.convertDpToPixel(15f));
        mThirdLinePaint.setTextSize(labelHeight);
        mThirdLinePaint.setTypeface(mXAxis.getTypeface());
        ...
        if (labels.length > 1) {
            ...
            if (labels.length >2){
                if (!TextUtils.isEmpty(labels[2])){
                    Utils.drawXAxisValue(c, labels[2], x, y + labelHeight + labelInterval+50, mThirdLinePaint, anchor, angleDegrees);
                }
            }
        } else {
